...
class OAPClientProxy(object):
    ...
    def process_message(self, message_json):
        ...
        mac = ''
        try:
            mac = self.format_mac_address(message['mac'])
            ...
            def oap_callback(mac_address, oap_resp):
                self.emit_event({
                    'event': 'result',
                    'id': request_id,
                    'mac': self.format_mac_address(mac_address),
    ...
    def to_byte_array_mac(self, mac):
        mac_address = None
        if isinstance(mac, list):
            mac_address = mac
        elif isinstance(mac, basestring):
...
if __name__ == "__main__":
    ...
    OAPClientProxy(serialport).start()
